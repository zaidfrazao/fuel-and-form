...
- **Created:** 2026-08-10
- **Last Updated:** 2026-09-22 — **§ P10's set rows show last time's reps (FUEL-122).** The circuits tell the reader to start at the bottom of each rep range and work up over the weeks, and the session state offered only the target, so working up meant remembering. Each set row now carries the reps logged for the same set the last time that exercise had any set. **The § Non-Goals row is amended rather than contradicted:** a personal record is a best across all history, and this is one earlier session's own number, with no max, trend, suggestion or beat-marking. **"Last time" is per exercise, not per workout.** Exercise rows belong to one workout, so this already means the same workout, and a partial session that never reached an exercise falls back to the time before it rather than showing nothing. **The tick's fallback is unchanged, in writing:** the placeholder and what an empty box logs stay the target's low rep, and last time is text beside the box. One criterion is added.
