...
**§ Data Model gains nothing here, deliberately.** The storage this needs is a table and a few columns, and every number in it — the cap on stored points, the coordinate precision, whether the first and last stretch of every trace is trimmed away from the front door — is a decision FUEL-100 makes with real sampling in front of it. Writing a schema into this section now would be this document guessing at figures it has no way to check, which is the failure § Data Model spent FUEL-89 recovering from. The listing gains its rows when they are built, the way `exercise_sets` did.
...
- [ ] The walk's energy estimate uses pace where a distance exists, falls back cleanly where it does not, is shown as a range, and is netted against `target_kcal` and every macro total nowhere
