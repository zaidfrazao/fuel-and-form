...
- **Per-set logging** — reps against an exercise, set by set, in a new table hanging off the session log rather than off the plan, because a set is history. A `load_kg` column ships dormant so the gym restart stays a data change. Structured targets arrive as nullable columns beside the prescription; the prescription string itself is still rendered verbatim and still never parsed, because a set count derived by regex from prose reads 8 for "8–12 rounds". **Each set row also shows what was done last time (FUEL-122):** the reps logged for the same set, the last time that exercise had any set before the date being viewed. This is recall, not a record. It is a number the reader already produced, shown beside the box where they will write the next one, which is what the circuits' "work up over the weeks" needs. It is not a best, a personal record, a trend or a suggestion, and nothing marks a set that beats it. A row with no earlier set shows nothing. The box's placeholder and what the tick logs from an empty box stay the target's low rep, so the box never offers one number while the tick records another. **A timed hold is logged in seconds, never as reps (FUEL-123).** The planks, the side plank and the superman are prescribed in seconds, and their set rows used to store a 45-second hold as 45 reps, which the export and the energy estimate then treated as reps. A structured seconds target now sits beside the reps target, on the same terms: transcribed by hand, never read off the prescription, and never beside a reps target on the same exercise. A set stores reps or seconds, exactly one, and which one follows the exercise's target on the server, never the client. Its row reads `Target 30–60s`, an empty tick logs 30 seconds, and last time is that set's seconds. Sets logged before this change are reps and read as they did. The intervals' round count is not in this change: rounds are not sets, and "8–12 rounds" still has no target of either kind.
- **A rest timer** between exercises. Client-only — nothing about a rest interval is worth a row — and counted from a stored end instant rather than an accumulated one, since the phone is locked for most of a ninety-second rest and a throttled tab stops counting. **Its presets are the rests the program writes, it takes a custom duration, and in a circuit a new set starts it (FUEL-126).** It was manual only, offering 60, 90 and 120 seconds, while the circuits ask for ~20 between exercises and 90 between rounds, and the intervals for 40 off. So the presets became 20, 40 and 90, with a box for seconds beside them. FUEL-93 ruled against auto-start because it "would start counting every time a set was corrected". The owner asked for it again, and that objection has an answer: a correction re-logs a set that is already there, so the reader's position cannot move and nothing starts. In a circuit, a newly logged set starts 20 seconds when the round goes on and 90 when it finishes one, and the last set starts nothing. Every other session keeps the manual timer, because only the circuits have a rest that `workouts.type` can stand for, and the format's prose is still never parsed. Stop works as it always did.
...
- **Form reference media** per exercise, shipped as bundled assets. No storage service and no third-party embeds, so § Integrations stays at "none" — and because this repository is public, no asset lands without a recorded licence it can carry. **Seen while browsing, not only on request (FUEL-129):** on `/training`'s exercise list, each row with a reference leads with a thumbnail of its working position, and the full reference (both frames, large) opens in a sheet from the row. `/` still loads no media, and the session state keeps its `Show form`. The thumbnails spend § P3's planning window the way the group headings do, and the Brand Guide's § Lists › The row's photograph records what they cost.
...
**These five arrive on a second state of the screen, not on top of the first.** Brand Guide § Desktop rules it (FUEL-90): `/training` keeps the plan state it has today, and gains a session state — reachable only for today, entered by the primary — where the measure holds the current exercise and its sets instead of the whole list, the rest timer rides in the action bar, and at ≥1272 the remaining exercises sit in the aside so the desktop shows both at once. It is a state rather than a route, § Navigation's two levels are untouched, and the current exercise is derived from the sets already logged rather than stored. **A circuit is stepped round by round (FUEL-119):** both circuits are three rounds of back-to-back exercises, so after set 1 of one exercise the state moves to set 1 of the next, and the round is named in words. What makes a session a circuit is `workouts.type`, never the format's prose, and the round is derived from the logged set numbers just as the exercise is. **The reader can move past an exercise, and back, without logging (FUEL-120):** otherwise an exercise finished short of its target held the state, and the only way on was a set that did not happen. Where they moved is kept in the browser beside the entered flag, never in the database, so the sets still say only what was performed and the status is untouched. **Skip session asks once a set is logged (FUEL-121):** in this state the third status reads `Skip session`, because a bare Skip under a list of sets reads as skipping the exercise, and one tap recorded a mostly-done session as skipped. With a working set logged it opens a confirmation sheet first. With none logged it records in one tap, so a session started by mistake still has an honest way out. The plan state's Skip is unchanged. The alternative — sets expanding in place under each row — is an accordion, which the Brand Guide has banned since before this milestone was written.
...
- [ ] The rest timer offers 20, 40 and 90 seconds and a custom duration; in a circuit a newly logged set starts 20 seconds within a round and 90 at its end, and a correction, a removal or the last set starts nothing (FUEL-126)
- [ ] Every shipped media asset has recorded provenance and a licence this public repository can carry; an exercise without media renders no affordance and no placeholder
- [ ] In a circuit (`workouts.type` = `circuit`) the session state steps round by round and names the round; a reload resumes on the same exercise and round with nothing new stored, and any other type steps exercise by exercise (FUEL-119)
- [ ] On `/training`'s exercise list, each exercise with media shows its working frame on its row without anything being opened; the row still opens the full reference; `/` loads no media (FUEL-129)
...
- [ ] Each set row in the session state shows the reps logged for that set the last time the exercise was trained, where one exists, and nothing where none does; the tick's fallback stays the target's low rep (FUEL-122)
- [ ] A timed exercise's set rows are in seconds, and ticking an empty row logs the target's low end in seconds; a timed set is stored as seconds and never as reps, and both export formats carry it in its own column (FUEL-123)
...
| An automatic progression or programming engine | The app records what was done; it does not decide what to do next. No prescribed load increases, no personal records, and no movement library beyond the workouts already in it. § P10 adds per-set logging and none of this. Its last-time figure (FUEL-122) is recall of the previous session's own reps, not a best and not a target. |
...
- **Created:** 2026-08-10
- **Last Updated:** 2026-09-23 — **§ P10's rest timer offers the program's rests, takes a custom one, and starts itself in a circuit (FUEL-126).** The presets were 60, 90 and 120 seconds. The rest the circuits ask for most, ~20 seconds between exercises, had none, and two of the three were rests the program never asks for. They are now 20, 40 and 90, with a box for seconds beside them. **Auto-start reverses FUEL-93's ruling, and it was the owner's call.** FUEL-93's objection was that an automatic timer would restart on every correction. A correction re-logs a set already there, so the position cannot move and nothing starts. That guarantee sits in the derivation, `restAfterLog`, not in which control fired. It is circuits only: 20 seconds within a round, 90 at its end, nothing after the last set. The ticked FUEL-93 criterion stands as written for what it tested, and a new criterion carries the change. Nothing is written to the database. The Brand Guide's v5.25 records the row.
- **Previously:** 2026-09-22 — **§ P10 logs a timed hold in seconds (FUEL-123).** The plank is prescribed "3 x 30–60 sec", and its set rows were the same reps box as every other exercise, so a 45-second hold was stored, exported and costed as 45 reps. **The shape follows § P10's own precedent for targets:** a seconds pair beside the reps pair, transcribed by hand for the four holds, with the prescription still never parsed. `exercise_sets` gains `seconds` beside a now-nullable `reps`, and a check requires exactly one. The server takes the unit from the exercise's target, not from the client. **The energy estimate prices a hold at its measured seconds**, because the rep rate never applied to it. **The export gains a column rather than reinterpreting one:** the week CSV's sets section has `seconds` after `reps`, and the JSON rows carry `seconds`, which is null on every set logged before this change. **Not in this change:** the intervals' round count, which is not a set and still has no target, and a start/stop hold timer, since a typed value is enough to fix the record. One criterion is added.
- **Previously:** 2026-09-22 — **§ P10's set rows show last time's reps (FUEL-122).** The circuits tell the reader to start at the bottom of each rep range and work up over the weeks, and the session state offered only the target, so working up meant remembering. Each set row now carries the reps logged for the same set the last time that exercise had any set. **The § Non-Goals row is amended rather than contradicted:** a personal record is a best across all history, and this is one earlier session's own number, with no max, trend, suggestion or beat-marking. **"Last time" is per exercise, not per workout.** Exercise rows belong to one workout, so this already means the same workout, and a partial session that never reached an exercise falls back to the time before it rather than showing nothing. **The tick's fallback is unchanged, in writing:** the placeholder and what an empty box logs stay the target's low rep, and last time is text beside the box. One criterion is added.
- **Previously:** 2026-09-21 — **§ P10's session state walks a circuit round by round (FUEL-119).** Both circuits say *3 rounds, each exercise back to back*, and the session state kept the reader on one exercise until its last set was ticked, which is straight sets. The prose is still never parsed: a session is a circuit when `workouts.type` says so, and the rounds are the exercises' own `target_sets`. No column is added and nothing new is stored. The Brand Guide's v5.18 records the rule and the redrawn session frames.
- **Previously:** 2026-09-15 — **§ P10's form media is seen while browsing the list, not only in a sheet (FUEL-129).** The owner found `/training` too text-heavy to scan. Nine rows of names, cues and prescriptions, with the pictures one tap away behind each row. A Mobbin comparison of training apps found a picture on every comparable exercise row. So the plan-state list gains a thumbnail of each reference's working position. The § P10 bullet says so, and one criterion is added. **One criterion is reworded rather than kept:** "no affordance and no gap" becomes "no affordance and no placeholder". The Brand Guide now gives the photograph's column to the group, so a row without media inside a group that has some keeps an empty column. That keeps the names aligned, and an empty column draws nothing. **Unchanged:** `/` loads no media, the session state keeps `Show form`, and the sheet still holds the full reference. The Brand Guide's v5.17 carries the rule and its costs.
- **Previously:** 2026-09-10 — **§ P6's file carries the walk, § P11's energy estimate is built, and the route/export question is closed by declining to build the opt-in (FUEL-104).** Three small things gathered into one ticket because each fails the same way, silently and by omission. **Two of the three were already true and were verified rather than rebuilt:** the JSON backup has carried distance, steps and source since FUEL-103 because `withoutUser` spreads the whole row on purpose, and the demo persona has had two walks a day with invented Greenwich Park routes since FUEL-100. Both now have tests that would notice if they stopped. **The real export gap was the weekly CSV**, whose training section stopped at `duration_min`; it gains `distance_m`, `steps` and `steps_source`, read off the log row rather than computed — `steps.convention.test.ts` bans the export builders from importing the module that derives a step count, and its own note already permitted them to carry the stored figure, so FUEL-103 had pre-authorised this and no allowlist needed widening. **`steps` stays bare rather than taking the file's `est_` prefix**, and the reason is recorded because the next reader will want to "fix" it: the column carries `steps_source` beside it, which says per row whether the count was measured or derived, and a blanket `est_steps` would assert the estimate of every row including a device count — flattening the exact distinction FUEL-103 built the source column to preserve. **The energy estimate is a pace where a distance exists and a wide band where it does not.** `WALK_PACE_POINTS` is the Compendium's walking METs at the paces they are quoted for, and a walk takes the band between the two points its pace falls between, so every bound printed is a compendium figure and the width of the range is the width of the bracket rather than an invented spread. A pace off either end of the table is treated as **a distance that cannot be believed** — a mistyped figure, or a recording that kept running in a car — and falls back to the duration rather than clamping to the top of the table, which would answer a broken row with a confident brisk-walk price. FUEL-95's constraints are inherited rather than re-implemented: the range, the outward rounding, the never-zero floor and the nearest-date bodyweight all come from the existing path, which is what makes them true by construction. **The one gap the ladder does not fill is a walk with no duration**, which has no minutes to price and is left at no estimate rather than at zero. **The estimate reaches the export and no screen, and the ticket assumed otherwise.** It said two walks a day would make this "the most frequently shown estimate in the app"; Brand Guide § The Route Trace had already closed the walk's figure list — distance, duration, pace, the step estimate and its source, the route's name, "with nothing invented for it" — and a burn range is not on it, so no placement was built and 220 visual baselines came back unchanged. Giving it one is a Brand Guide amendment first. **`export-energy.test.ts` gains a walk**, because the export is therefore the only place the figure exists and, at two walks a day, the estimate it carries most often — a fixture holding only a circuit would have kept passing while it went unchecked; its figures are numerically tuned like `DURATION_MIN` before them, a 50-minute walk having put `250 - 78` exactly on `heightCm`. **Routes stay out of the export and no opt-in was built**, which is the decision rather than the absence of one: an opt-in is a control whose ON state writes a home address into a mailed file, and every later export change would then have to ask whether this was the run where that happened. `walk_routes.name` is covered by the same table-level exclusion, deliberately — free text is the half that looks safe, and schema.ts refuses that reading. The structural check is backed by a byte-level scan whose plant runs first, and the planted pair is COMPUTED: `check-no-metrics.sh` says its pair pattern has no allowlist and must never be given one, and it caught the literal on the first attempt.
- **Previously:** 2026-09-09 — **§ Data Model gains `workout_logs.steps` and `steps_source`, and § P11's step figure is built as an estimate that says so (FUEL-103).** No rule changes here; the section has specified this figure since FUEL-99 and the app catches up. **The coefficient is STEP length, not stride**, and the distinction is recorded because § P11's own wording invites the error: a stride is two steps, the words are used interchangeably in casual writing, and taking the stride ratio would have halved every count in the app while leaving a figure that still looked plausible. The check that catches it is steps-per-kilometre against the usual band, and it is a test rather than a comment. **The precision is two significant figures**, which is the honest reading of "a precision that matches its accuracy": rounding to the nearest hundred was the obvious alternative and is wrong at exactly the walks this app records, being 1% at four thousand steps and 50% at one hundred. **The source column is the point of the ticket and the estimate is the smaller half.** `steps_source` declares `'device'` today with nothing able to write it — FUEL-105 is the deferred native companion that could — and the rule that a device count is never overwritten by a re-estimate lives in the upsert's own `DO UPDATE`, where it is atomic, rather than in a read-then-write beside it. That branch is unreachable from every path a user can take, so the integration test **plants the row itself**; a suite that only ever wrote estimates would pass against an implementation that clobbers, and the planted break confirmed exactly one test fails when it does. **Both columns are null or neither is**, under a CHECK, because a count whose origin is unnamed is the confusion the source column exists to prevent. **The cost is stated rather than discovered:** the estimate is stored, so correcting `profiles.height_cm` does not re-estimate past walks — deliberate, since a settings save that silently rewrote every historical figure is the worse failure, and it would rewrite device counts too. § P11's "no daily step total" is enforced rather than merely absent: `steps.convention.test.ts` bans the import from every module that reports a day or a week, and — because this figure is STORED where the energy estimate is computed — a second ban stops the aggregators naming a step field at all. **The export is FUEL-104's** and is not built here; the columns reach the JSON backup regardless, because it serialises the whole row.
