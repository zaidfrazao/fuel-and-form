...
**`cursor: pointer` on everything interactive that is not a `<button>` or `<a>` by nature** — list rows, tiles, checkboxes and week cells. Elements that already carry it from the user agent are left alone. It is the smallest rule here and the one most often missed, because it is invisible to the keyboard the developer is testing with.
