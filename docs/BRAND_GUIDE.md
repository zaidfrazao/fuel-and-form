...
| **Training** | `/training` | The date paginator across the frame; the measure keeps the session, the exercise list and the note; the aside keeps the dot grid and recent sessions |
...
The mock still does not draw them, and that stays true: the mock's seven are not the app's seven. What changes is that a ruling in prose is now specific enough to build from.
