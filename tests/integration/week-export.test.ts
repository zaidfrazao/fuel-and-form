...
describe.skipIf(!configured)("the weekly export, scoped", () => {
  ...
  it("shows a neighbouring week's plan without its logs", async () => {
    ...
    // Scheduled, unlogged: a blank status, a blank duration, the three walk
    // columns blank because a circuit has none of them, and — since nothing
    // happened — two blank `est_burn` cells.
